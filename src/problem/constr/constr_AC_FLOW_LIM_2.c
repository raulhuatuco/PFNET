/** @file constr_AC_FLOW_LIM_2.c
 *  @brief This file defines the data structure and routines associated with the constraint of type AC_FLOW_LIM_2.
 *
...
#include <pfnet/constr_AC_FLOW_LIM_2.h>

#define JACOBIAN_VAL() 2*(R*dRdx+I*dIdx)
#define HESSIAN_VAL() 2*(dRdy*dRdx+dIdy*dIdx+R*d2Rdydx+I*d2Idydx)

Constr* CONSTR_AC_FLOW_LIM_2_new(Net* net) {
  Constr* c = CONSTR_new(net);
  CONSTR_set_func_init(c, &CONSTR_AC_FLOW_LIM_2_init);
  CONSTR_set_func_count_step(c, &CONSTR_AC_FLOW_LIM_2_count_step);
  CONSTR_set_func_allocate(c, &CONSTR_AC_FLOW_LIM_2_allocate);
  CONSTR_set_func_clear(c, &CONSTR_AC_FLOW_LIM_2_clear);
  CONSTR_set_func_analyze_step(c, &CONSTR_AC_FLOW_LIM_2_analyze_step);
  CONSTR_set_func_eval_step(c, &CONSTR_AC_FLOW_LIM_2_eval_step);
  CONSTR_set_func_store_sens_step(c, &CONSTR_AC_FLOW_LIM_2_store_sens_step);
  CONSTR_set_func_free(c, &CONSTR_AC_FLOW_LIM_2_free);
  CONSTR_init(c);
  return c;
}
...
void CONSTR_AC_FLOW_LIM_2_init(Constr* c) {
  ...
  CONSTR_set_name(c,"squared AC branch flow limits");
...
void CONSTR_AC_FLOW_LIM_2_analyze_step(Constr* c, Branch* br, int t) {
  ...
  for (k = 0; k < 2; k++) {
    ...
    VEC_set(CONSTR_get_u_extra_vars(c),*J_row,BRANCH_get_ratingA(br)*BRANCH_get_ratingA(br));
    ...
    MAT_set_d(G,*J_row,1.);
    VEC_set(l,*J_row,0.);
    VEC_set(u,*J_row,BRANCH_get_ratingA(br)*BRANCH_get_ratingA(br));
...
void CONSTR_AC_FLOW_LIM_2_eval_step(Constr* c, Branch* br, int t, Vec* values, Vec* values_extra) {
